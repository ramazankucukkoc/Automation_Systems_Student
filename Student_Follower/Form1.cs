...
namespace Student_Follower
{
    public partial class Form1 : Form
    {
        ...
        {
            CreateUser userAdd = new CreateUser();
            userAdd.FirstName = textBox1.Text;
            userAdd.LastName = textBox2.Text;
            userAdd.Email = textBox3.Text;
            userAdd.Password = textBox4.Text;

                bool IsAdd = await _userApiService.Add(userAdd);
                if (IsAdd)
                {
                    MessageBox.Show("Öğrenci başarıyla eklendi.", "Ögrenci Takip Otomasyonu", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    MessageBox.Show($"{userAdd.Email} adresi mevcuttur farklı bir email girin", "Ögrenci Takip Otomasyonu", MessageBoxButtons.OK, MessageBoxIcon.Error);
